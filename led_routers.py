...
def override_brightness(
    ...
    """
    Insert a manual-override brightness entry for a given LED.
    """
    # Optional guard: make sure the fixture exists
    ...
    row = models.BrightnessLevel(
        ...
        level=payload.level,
        ts=datetime.utcnow(),           # explicit, or rely on DB default
    )
    db.add(row)
    db.commit()
    db.refresh(row)                     # get generated id + ts
    return row